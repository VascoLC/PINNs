...
def apply_bc_u(st, ctx):
    # Extract indices and grid sizes from context
    _, ix, iy = ctx.indices()
    _, nx, ny = ctx.size()
    mod = ctx.mod
    # Get y-coordinates (assuming normalized domain [0,1])
    y_vals = mod.cast(iy / (ny - 1), dtype=st[0][0].dtype)  # Shape: (Ny,)
    
    # Compute triangular profile for right boundary
    tri = mod.where(y_vals <= 0.5, y_vals / 0.5, (1 - y_vals) / 0.5)
    
    # Apply boundary conditions
    # x-direction: Dirichlet (u=0 on left, triangular on right)
    uc, um, up = st[0]
    st[0][1] = mod.where(ix == 0, -uc, um)     # Left boundary: u=0
    st[0][2] = mod.where(ix == nx-1,- uc, up)  # Right boundary: u=0
...
def operator_odil(ctx):
    ...
    # Get stencils
    u_st = stencil_var("u")
    u_st = apply_bc_u(u_st, ctx)  # Modified to pass ctx
    uf_st = stencil_var("u", frozen=True)
    uf_st = apply_bc_u(uf_st, ctx)  # Modified to pass ctx
...
def parse_args():
    ...
    parser.add_argument("--ref_path",type=str, default="out_heat2D_direct_final_step/ref_solution.pickle",
    ...
    parser.add_argument("--nimp", type=int, default=1000, help="Number of points for imposed=random")
    ...
    parser.add_argument("--src_strength", type=float, default=1.0,
                    help="Amplitude of moving heat source")
    parser.add_argument("--src_speed",    type=float, default=0.5,
                        help="Speed of source in +x direction")
    parser.add_argument("--src_x0",       type=float, default=0.5,
                        help="Initial x‑center of source")
    parser.add_argument("--src_y0",       type=float, default=0.5,
                        help="y‑center of source")
    parser.add_argument("--src_width",    type=float, default=0.1,
                        help="Full width of the moving stripe in x")
    odil.util.add_arguments(parser)
    odil.linsolver.add_arguments(parser)
    parser.set_defaults(outdir="out_heat2D_inverse_final_step_attempt_frozen1")
    ...
    parser.set_defaults(plot_every=250, report_every=500, history_full=10, history_every=100, frames=10)
