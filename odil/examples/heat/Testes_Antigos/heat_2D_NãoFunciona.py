...
def get_tri_bc(x, x_peak=0.5, width=1.0, amplitude=1.0, mod=np):
...
def operator_odil(ctx):
    ...
    def apply_bc_u(st):
        ...
        for q in st:
            # West boundary (x=0): zero Dirichlet
            q[1] = mod.where(ix == 0, extrap(q[2], q[0], 0.0), q[1])

            # East boundary (x=nx-1): triangular profile
            q[2] = mod.where(ix == nx - 1, extrap(q[1], q[0], 0.0), q[2])

            # South boundary (y=0): zero Neumann
            q[3] = mod.where(iy == 0, q[0], q[3])

            # North boundary (y=ny-1): zero Neumann
...
def parse_args():
    ...
    parser.add_argument("--ref_path", help="Path to reference solution *.pickle")
    parser.add_argument(
        "--imposed",
        type=str,
    ...
    parser.set_defaults(outdir="out_heat_direct_2D_32_Step2")
...
def make_problem(args):
    ...
    if imp_size > 0:
        print(f"Exporting {imp_size} imposed points to CSV...")
        
        # We need the pandas library for this
        try:
            import pandas as pd
        except ImportError:
            print("Warning: pandas is not installed. Skipping CSV export.")
            return # Or handle the error as you see fit

        # Get the shape of the 3D grid (time, x, y)
        Nt_grid, Nx_grid, Ny_grid = domain.cshape

        # Convert the flattened `imp_indices` to 3D grid indices (t_idx, x_idx, y_idx)
        # This is a 3D extension of the logic from your 1D script.
        t_indices_np = imp_indices // (Nx_grid * Ny_grid)
        remainder = imp_indices % (Nx_grid * Ny_grid)
        x_indices_np = remainder // Ny_grid
        y_indices_np = remainder % Ny_grid

        # Convert numpy indices to TensorFlow tensors for indexing
        t_indices_tf = tf.constant(t_indices_np, dtype=tf.int32)
        x_indices_tf = tf.constant(x_indices_np, dtype=tf.int32)
        y_indices_tf = tf.constant(y_indices_np, dtype=tf.int32)

        # Stack the indices into a single (N, 3) tensor for tf.gather_nd
        indices_for_gather = tf.stack([t_indices_tf, x_indices_tf, y_indices_tf], axis=-1)
        
        # `imp_u` is the 3D grid of u-values. It's a TensorFlow tensor.
        # Use tf.gather_nd to extract the u-values at the specified indices.
        u_imposed_values_tf = tf.gather_nd(imp_u, indices_for_gather)
        
        # Convert the result back to a NumPy array for use with pandas
        u_imposed_values = u_imposed_values_tf.numpy()

        # `imp_points` is already an (N, 3) NumPy array with columns (t, x, y)
        ...
        output_csv_path = "imposed_data_with_u_2D.csv"
        df_imposed_data.to_csv(output_csv_path, index=False)
        print(f"Successfully saved imposed data to '{output_csv_path}'")
