...
def operator_odil(ctx):
    ...
    if args.infer_s:
        ...
        v  = args.src_speed
        x0 = args.src_x0
        y0 = args.src_y0  # new argument already in your parser

        # --- Gaussian blob parameters ---
        sigma_x = args.src_width / 2.355  # converts FWHM -> standard deviation
        sigma_y = sigma_x  # same width in y, can make independent if desired

        # --- Moving Gaussian blob ---
        x_center = x0 + v * t
        y_center = y0  # stationary in y

        # Get coordinates from domain
        y = dom.lower[2] + ctx.cast(iy) * dy

        # 2D Gaussian in (x, y)
        S = A * mod.exp(-(((x - x_center)**2) / (2 * sigma_x**2)
...
def parse_args():
    ...
    parser.set_defaults(outdir="out_heat_direct_Gaussian")
    ...
    parser.set_defaults(plot_every=1000, report_every=100, history_full=10, history_every=100, frames=10)
...
def make_problem(args):
    ...
    u_np = np.array(ref_u)
    u_values = u_np.flatten()[imp_indices]
    k_values = get_ref_k(u_values)

    with open("imposed.csv", "w") as f:
        f.write(",".join(domain.dimnames) + ",u,k\n")
        for p, u_val, k_val in zip(imp_points, u_values, k_values):
            f.write(f"{p[0]:.8f},{p[1]:.8f},{p[2]:.8f},{u_val:.8f},{k_val:.8f}\n")
