...
def parse_args():
    ...
    parser.set_defaults(outdir="out_heat_inverse_1D_testing_LBFGSB")
...
def main():
    ...
    if args.infer_k:
        printlog("\nSaving trained k_net weights...")
        
        # Access the trained Keras model from the ODIL state object
        k_net_model = state.fields["k_net"].array
        
        # Define the output filename
        weights_filename = "k_net_pretrained_weights.h5"
        
        # Use the standard Keras function to save the weights
        k_net_model.save_weights(weights_filename)
        
        printlog(f"Successfully saved k_net weights to '{weights_filename}'")
