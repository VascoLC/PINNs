...
# ------------------------------
# 5) (Optional) Domain samples for PDE residual
# ------------------------------
...
vars = net_u.trainable_variables + net_k.trainable_variables

# Function to pack variables into a 1D numpy array
def pack_weights():
    return np.concatenate([v.numpy().flatten() for v in vars])

# Function to unpack a 1D numpy array back into the variables
def unpack_weights(x):
    pointer = 0
    for v in vars:
        shape = v.shape
        size = np.prod(shape)
        v.assign(x[pointer : pointer + size].reshape(shape))
        pointer += size

# Objective for L-BFGS: returns loss and gradient
def objective(x):
    unpack_weights(x)
    with tf.GradientTape() as tape:
    ...
    return loss_value.numpy().astype(np.float64), grad_flat

# Initial guess from Adam-trained weights
x0 = pack_weights()
...
res = opt.minimize(
    ...
    method='L-BFGS-B',
    jac=True,
    options={'maxiter': 500, 'maxcor': 50, 'ftol': 1e-8}
)

# Assign the optimized weights back to the model
unpack_weights(res.x)
print("L-BFGS optimization finished. Final loss:", res.fun)


'''
# Save the models
net_u.save("net_u_model", save_format="tf")
net_k.save("net_k_model", save_format="tf")

# To load later
#from tensorflow.keras.models import load_model
#net_u = load_model("net_u_model", custom_objects={"NetU": NetU})
#net_k = load_model("net_k_model", custom_objects={"NetK": NetK})
...
X, Y = np.meshgrid(x, y)
#T = np.ones_like(X)  # t =  everywhere
#XYT_grid = np.stack([X.ravel(), Y.ravel(), T.ravel()], axis=1).astype(np.float32)

# 2) Run the networks directly (no .predict)
#XYT_tensor = tf.convert_to_tensor(XYT_grid)
#u_tensor = net_u(XYT_tensor, training=False)    # shape (nx*ny, 1)
#k_tensor = net_k(u_tensor,    training=False)   # shape (nx*ny, 1)

# 3) Convert to NumPy and reshape to 2D fields
