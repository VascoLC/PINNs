...
df = df_1.sample(n=1024, random_state=42)

# If I want to use *all* of them:
...
# Initial condition: u(x,y,0) = g(x)-g(0)
n_ic = 400
...
# Boundary condition: u = 0 on ∂Ω for t ∈ [0,1]
n_bc = 400
...
for epoch in range(1, epochs+1):
    ...
    for batch in range(num_batches):
        ...
        XT_obs_mb = tf.convert_to_tensor(XT_obs_np[batch_idx])
        ...
        with tf.GradientTape() as tape:
            total_loss, loss_u, loss_k = compute_loss(
                ...
                xt_obs    = XT_obs_mb, u_obs = u_obs_mb, k_obs = k_obs_mb,
            )
...
# 2) Flatten into (Nx*Nt,2) for the PINN
...
cf = plt.contourf(Xg, Tg, u_pred, levels=100, cmap="inferno")
